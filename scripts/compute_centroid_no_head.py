...
def compute_keypoint_variations(keypoints):
    ...
    numpy.ndarray
        ...
        or -np.inf for invisible or predefined keypoints
    """
    keypoints = np.array(keypoints).reshape(-1, 3)
    predefined_ignore = {1, 2, 3, 8, 11, 14, 17}  # Keypoints to ignore (1-based index)
    # 7 10 13 16 for removing/ adding knee and elbows
    visible = keypoints[:, 2] > 0  # Mask for visible keypoints
    for kp in predefined_ignore:
        visible[kp - 1] = False  # Convert to 0-based indexing and mask them
    ...
    distances = np.full(len(keypoints), -np.inf)  # Default to -inf for ignored/invisible keypoints
    ...
    # Set non-visible or predefined keypoints to -inf
    variations[~visible] = -np.inf
...
def main():
    ...
    output_dir = os.path.join(base_path, "centroid_variation_no_head")
